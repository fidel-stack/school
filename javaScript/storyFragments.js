...
function sortFragments(fragments) {
  ...
  for (const fragment of fragments) {
    let index = 0;
    while(index < result.length && result[index].id <= fragment.id){
      index++;
    }
    result.splice(index, 0, fragment)
  }
  return result;
}
...
function fillMissingFragments (fragments) {
  const result = [];
  for (let i = 0; i < fragments.length; i++) {
    if (i > 0 && fragments[i].id - fragments[i - 1].id > 1) {
      for (let missingId = fragments[i - 1].id + 1; missingId < fragments[i].id; missingId++){
        result.push({
          id: missingId,
          text: '[...]'
        })
